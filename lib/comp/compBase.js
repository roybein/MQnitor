...
PlanBase.prototype.addPlan = function(plan) {
    ...
    this.collec.update({_id:plan.index}, {_id:plan.index, name:plan.name, relayId:plan.relayIndex, relayValue:plan.relayValue, judgeGroup: plan.judgeGroup}, {upsert:true});
}

PlanBase.prototype.getPlan = function(index) {
    var planDb = this.collec.findOne({_id:index});
    var plan = new Plan(planDb._id, planDb.name, planDb.relayId, planDb.relayValue, planDb.judgeGroup); 
    return plan
}

PlanBase.prototype.attachPlan = function(relayIndex, planValue, planIndex) {
    relayAll.collec.update({_id:relayIndex}, {$set:{planId:planIndex, planVal:planValue}});
